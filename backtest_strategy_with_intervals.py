...
def calculate_indicators(df):
    ...
    for i in range(len(df)):
        if df.loc[i, '扬帆起航']:
            ...
            current_interval = '震荡'
        df.loc[i, '信号区间'] = current_interval
    
    return df

def backtest_stock(df, stock_code, stock_name, market_df):
    """对单只股票进行回测（带信号区间和大盘过滤）"""
    df = df.copy()
    df = calculate_indicators(df)
    
    # Merge with market signals (on date)
    market_df_reindexed = market_df.set_index('日期')[['信号区间']].add_prefix('market_').reset_index()
    df = pd.merge(df, market_df_reindexed, on='日期', how='left')
    df['market_信号区间'] = df['market_信号区间'].fillna('未知')
    ...
    for phase_name, (start_date, end_date) in phases.items():
        ...
        for i in range(len(phase_df)):
            ...
            if i > 0:
                ...
                # 买入信号：
                # 前一天个股在买入区间，且大盘在买入或震荡区间，且我们有现金
                if prev['信号区间'] == '买入' and prev['market_信号区间'] in ['买入', '震荡'] and cash > 0:
                ...
                # 1. 前一天个股在卖出区间
                # OR
                # 2. 前一天大盘在卖出区间，且个股持仓有盈利
                sell_signal = False
                if prev['信号区间'] == '卖出' and total_shares > 0:
                    sell_signal = True
                elif prev['market_信号区间'] == '卖出' and total_shares > 0 and current_profit > 0:
                    sell_signal = True
...
def main():
    ...
    # First calculate market (SH#999999) indicators with intervals
    print("Calculating market (SH#999999) indicators with intervals...")
    ...
    for stock_code, group in stock_groups:
        ...
        print(f"正在回测: {stock_code} - {stock_name} (带信号区间)")
    ...
    results_df.to_csv('backtest_results_with_intervals.csv', index=False, encoding='utf-8-sig')
    
    trades_df = pd.DataFrame(all_trades)
    trades_df.to_csv('backtest_trades_with_intervals.csv', index=False, encoding='utf-8-sig')
    
    print("\n" + "="*80)
    print("带信号区间的回测完成！")
    print(f"共回测 {len(stock_groups)-1} 只股票")
    print(f"结果已保存到: backtest_results_with_intervals.csv 和 backtest_trades_with_intervals.csv")
    print("="*80)
