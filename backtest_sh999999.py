                ...
                if prev['扬帆起航'] and position == 0:
                    ...
                    if pd.notna(open_price) and open_price > 0:
                        ...
                        if shares > 0:
                            ...
                            trades.append(trade)
                            all_trades.append({**trade, '股票代码': stock_code, '股票名称': stock_name, '阶段': phase_name})
                
                # 卖出信号：前一天收盘出现避风靠岸，今天开盘卖出
...
def main():
    ...
    try:
        results, df_with_indicators, all_trades = backtest_stock(sh_df, stock_code, stock_name)
        ...
        results_df.to_csv('sh999999_backtest_results.csv', index=False, encoding='utf-8-sig')
        
        trades_df = pd.DataFrame(all_trades)
        trades_df.to_csv('sh999999_backtest_trades.csv', index=False, encoding='utf-8-sig')
        
        print("\n" + "="*80)
        print("上证指数回测完成！")
        print(f"结果已保存到: sh999999_backtest_results.csv 和 sh999999_backtest_trades.csv")
        print("="*80)
        ...
        for phase in ['牛市', '震荡', '熊市']:
            print(f"\n【{phase}】")
            phase_result = results_df[results_df['阶段'] == phase].iloc[0]
            
            print(f"交易天数: {phase_result['交易天数']}")
            print(f"交易次数: {phase_result['交易次数']}")
            print(f"胜率: {phase_result['胜率']}%")
            print(f"总收益: {phase_result['总收益']}%")
            print(f"最大回撤: {phase_result['最大回撤']}%")
            print(f"年化收益: {phase_result['年化收益']}%")
            print(f"初始资金: {phase_result['初始资金']}")
            print(f"最终资金: {phase_result['最终资金']}")
            ...
            if len(phase_trades) > 0:
                ...
                for trade in phase_trades:
                    trade_type = trade['类型']
                    price = trade['价格']
                    if trade_type in ['卖出', '结算']:
                        profit = trade.get('利润', 0)
                        profit_pct = trade.get('利润率', 0)
                        print(f"  {trade['日期'].date()}: {trade_type} @ {price:.2f}, 利润: {profit:.2f} ({profit_pct:.2f}%)")
                    else:
                        print(f"  {trade['日期'].date()}: {trade_type} @ {price:.2f}")
