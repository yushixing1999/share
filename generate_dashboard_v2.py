...
# --- 读取数据 ---
print("Reading backtest data (with intervals)...")
results_df = pd.read_csv('backtest_results_with_intervals.csv')
trades_df = pd.read_csv('backtest_trades_with_intervals.csv')
...
from backtest_strategy_with_intervals import calculate_indicators
...
for stock_code, group in stock_groups:
    ...
    for phase in ['牛市', '震荡', '熊市']:
        ...
        phase_prices = df_with_indicators[mask][['日期', '收盘', '信号区间']].copy()
        phase_prices['日期'] = phase_prices['日期'].dt.strftime('%Y-%m-%d')
        all_data['prices'][str(stock_code_int)][phase] = phase_prices.rename(columns={'收盘': 'close', '信号区间': 'interval'}).to_dict('records')
...
# --- 读取HTML模板并嵌入数据 ---
print("Generating HTML dashboard v2...")
...
new_html = html_template.replace(
    'allData = {data_json};',  # 直接替换，不用loadSampleData()
    f'allData = {data_json};'
).replace(
    'loadSampleData();',
    ''  # 移除loadSampleData()调用
...
# 保存
with open('backtest_dashboard_v2.html', 'w', encoding='utf-8') as f:
    f.write(new_html)

print("Dashboard v2 generated successfully! Saved as backtest_dashboard_v2.html")
