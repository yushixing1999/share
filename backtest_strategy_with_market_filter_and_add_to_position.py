    ...
    for phase_name, (start_date, end_date) in phases.items():
        ...
        for i in range(len(phase_df)):
            ...
            if total_shares > 0:
                ...
                portfolio_value = cash + total_shares * current['收盘']
            else:
            ...
            if i > 0:
                ...
                avg_cost = total_cost / total_shares if total_shares > 0 else 0
                if total_shares > 0:
                    current_profit = (current['收盘'] - avg_cost) * total_shares
                
                # 买入信号：前一天收盘出现扬帆起航，今天开盘买入/加仓，且大盘不在避风靠岸状态
                if prev['扬帆起航'] and not prev['market_避风靠岸']:
                    open_price = current['开盘']
                    if pd.notna(open_price) and open_price > 0 and cash > 0:
                        # 每次用剩余现金的一部分买入（这里用全部剩余现金）
                        buy_shares = int(cash / open_price / 100) * 100  # 买整手
                        if buy_shares > 0:
                            ...
                            trades.append({
                                '日期': current_date,
                                '类型': '买入' if total_shares == buy_shares else '加仓',
                                '价格': open_price,
                                '数量': buy_shares,
                                '金额': cost,
                                '累计持仓': total_shares,
                ...
                # 卖出信号：
                # 1. 前一天收盘出现避风靠岸（个股信号），今天开盘卖出全部持仓
                # OR
                # 2. 前一天大盘出现避风靠岸信号，且当前有盈利，今天开盘卖出全部持仓
                sell_signal = False
                if prev['避风靠岸'] and total_shares > 0:
                    sell_signal = True
                elif prev['market_避风靠岸'] and total_shares > 0 and current_profit > 0:
                    sell_signal = True
                    
                if sell_signal:
                    open_price = current['开盘']
                    if pd.notna(open_price) and open_price > 0:
                        revenue = total_shares * open_price
                        cash += revenue
                        ...
                        trades.append({
                            ...
                            '利润': profit,
                            '利润率': profit_pct,
                            '持仓天数': (current_date - trades[0]['日期']).days if len(trades) > 0 else 0
                        })
        ...
        # 最后如果还有持仓，按最后收盘价结算
        if total_shares > 0 and len(phase_df) > 0:
            last = phase_df.iloc[-1]
            close_price = last['收盘']
            if pd.notna(close_price) and close_price > 0:
                revenue = total_shares * close_price
                cash += revenue
                profit = revenue - total_cost
                avg_cost = total_cost / total_shares if total_shares > 0 else 0
                profit_pct = (profit / total_cost) * 100 if total_cost > 0 else 0
                ...
                trades.append({
                    ...
                    '利润': profit,
                    '利润率': profit_pct,
                    '持仓天数': (last['日期'] - trades[0]['日期']).days if len(trades) > 0 else 0
                })
        
        # 计算统计指标
        buy_trades = [t for t in trades if t['类型'] in ['买入', '加仓']]
...
def main():
    ...
    for stock_code, group in stock_groups:
        ...
        print(f"正在回测: {stock_code} - {stock_name} (允许加仓)")
    ...
    results_df.to_csv('backtest_results_with_market_filter_and_add_to_position.csv', index=False, encoding='utf-8-sig')
    
    trades_df = pd.DataFrame(all_trades)
    trades_df.to_csv('backtest_trades_with_market_filter_and_add_to_position.csv', index=False, encoding='utf-8-sig')
    
    print("\n" + "="*80)
    print("带大盘过滤且允许加仓的回测完成！")
    print(f"共回测 {len(stock_groups)-1} 只股票")
    print(f"结果已保存到: backtest_results_with_market_filter_and_add_to_position.csv 和 backtest_trades_with_market_filter_and_add_to_position.csv")
    print("="*80)
    
    # 打印汇总结果
    print("\n" + "="*80)
    print("带大盘过滤且允许加仓的回测结果汇总")
    print("="*80)
